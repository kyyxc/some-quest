...
// Tambahkan tipe props agar tidak error
type AuthUser = {
    id: number;
    name: string;
    email: string;
...
const Sidebar = () => {
    ...
    const { logout, user, setUser } = useLoginStore();
    ...
    return (
        <div
            className={`sticky top-0 h-screen flex-col border-r border-gray-300 bg-white py-4 shadow-sm transition-all duration-300 md:flex ${collapsed
                ? 'w-16 min-w-[4rem] px-2'
                : 'w-64 min-w-[16rem] px-4'
                }`}
        >
            {/* Header */}
                    ...
                    return (
                        ...
                        >
                            ...
                            {!collapsed && item.badge && (
                                <Badge className="rounded-xs bg-gray-100 px-2 text-xs">
                                    {item.badge}
                                </Badge>
                            )}
                        </Link>
            ...
            <div className="mt-auto border-t border-gray-300 pt-4">
                <Button
                    ...
                    disabled={collapsed}
                    onClick={() => console.log('button setting')}
                >
                    ...
                    {!collapsed && (
                        <div className="flex flex-col items-start text-left leading-tight">
                            ...
                            <span className="text-xs text-muted-foreground break-all">{user?.email || 'No Email'}</span>
                        </div>
                    )}
                </Button>
